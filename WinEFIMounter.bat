...
if /i "%ch1%"=="q" set "ch1=" &goto :mounted_quit
...
mkdir "%temp%\WinEFIMounter"
xcopy /e /Y "C:\EFIPartition\*" "%temp%\WinEFIMounter\" > nul 2>&1
:errlcheck
if "%errl%"=="0" (
	set /a cpf=0
	set /a cnt=0
	set /a errl=0
	rmdir /s /q "%temp%\WinEFIMounter" > nul 2>&1
) else (
...
set /a cnt=0
set /a errl=0
if exist "%temp%\WinEFIMounter" rmdir /s /q "%temp%\WinEFIMounter" > nul 2>&1
